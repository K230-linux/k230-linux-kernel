...
static struct k230_gpio_port *k230_offs_to_port(struct k230_gpio *gpio,
						unsigned int offs)
{
...
static void k230_irq_enable(struct irq_data *d)
{
	...
	raw_spin_lock_irqsave(&gc->bgpio_lock, flags);
	val = k230_read(gpio, GPIO_INTEN) | BIT(irqd_to_hwirq(d));
				 ...
				 unsigned long config)
{
	if (pinconf_to_config_param(config) == PIN_CONFIG_INPUT_DEBOUNCE)
		return k230_gpio_set_debounce(gc, offset,
...
static int k230_gpio_probe(struct platform_device *pdev)
{
	...
	gpio->ports = devm_kcalloc(dev, gpio->nr_ports,
