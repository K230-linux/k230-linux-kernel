				   ...
				   struct drm_display_mode *adj_mode)
{
	...
	u32 plane_offset = config->plane_offset;
	u32 plane_enable_bit = config->plane_enable_bit;
	u32 xctl_reg_offset = config->xctl_reg_offset;
	u32 yctl_reg_offset = config->yctl_reg_offset;
	u32 actual_w, start_w, offset_w;
	u32 actual_h, start_h, offset_h;
	u32 y_addr, uv_addr;
	u32 stride = 0x00;
	u32 disp_en = 0x00;
	...
	case DRM_FORMAT_NV61:
		...
		break;
	default:
		dev_err(vo->dev, "Invalid pixel format %d\n",
			fb->format->format);
		return;
	}
	writel(reg_val,
			  ...
			  struct drm_plane_state *plane_state)
{
	...
	if (IS_ERR(crtc_state)) {
		dev_err(vo->dev, "Failed to get crtc_state\n");
	...
	ret = drm_atomic_helper_check_plane_state(plane_state, crtc_state,
						  ...
						  true);
	if (ret) {
		dev_err(vo->dev, "Failed to check plane_state\n");
		return ret;
	}
...
static irqreturn_t canaan_vo_irq_handler(int irq, void *dev_id)
{
	...
	/*
	 * The IRQ status register aggregates the IRQ0/1/2 sources. The exact
	 * per-bit layout is not documented by the vendor, so treat a wholly
	 * zero status as "not ours" and bail without acking, which keeps the
	 * line usable if it is ever shared.
	 */
...
int canaan_vo_enable_vblank(struct canaan_vo *vo)
{
	...
	canaan_vo_set_vtth_intr(vo, 1, vo->vtth_line);
...
void canaan_vo_disable_vblank(struct canaan_vo *vo)
{
	...
	canaan_vo_set_vtth_intr(vo, 0, vo->vtth_line);
	canaan_vo_write(vo, VO_REG_LOAD_CTL, 0x11);
}
...
static void canaan_vo_init(struct canaan_vo *vo)
{
	...
	for (i = 0; i < 8; i++)
		canaan_vo_write(vo, VO_DMA_WEIGHT_RD_BASE + (4 * i), 0x1010101);

	canaan_vo_write(vo, VO_DMA_PRIORITY_RD0_L, 0x76543210);
	canaan_vo_write(vo, VO_DMA_PRIORITY_RD_0, 0x76543210);
	canaan_vo_write(vo, VO_DMA_PRIORITY_RD2_L, 0x76543210);
	canaan_vo_write(vo, VO_DMA_ID_RD_0, 0x76543210);

	canaan_vo_write(vo, VO_DMA_PRIORITY_RD0_H, 0xfedcba98);
	canaan_vo_write(vo, VO_DMA_PRIORITY_RD_1, 0xfedcba98);
	canaan_vo_write(vo, VO_DMA_PRIORITY_RD2_H, 0xfedcba98);
	canaan_vo_write(vo, VO_DMA_ID_RD_1, 0xfedcba98);

	for (i = 0; i < 10; i++)
		canaan_vo_write(vo, VO_LAYER0_LINE0_BD_CTL + (4 * i), 0x701);

	canaan_vo_write(vo, VO_OSD3_BD_CTL, 0x701);
	canaan_vo_write(vo, VO_OSD4_BD_CTL, 0x701);
	canaan_vo_write(vo, VO_OSD5_BD_CTL, 0x701);
	canaan_vo_write(vo, VO_OSD6_BD_CTL, 0x701);
	canaan_vo_write(vo, VO_OSD7_BD_CTL, 0x701);
	...
	canaan_vo_write(vo, VO_DISP_MIX_OSD8_GLB_ALPHA, 0xffffffff);
	...
	canaan_vo_write(vo, VO_DISP_MIX_SEL1, reg);
	...
	/* Disable all VO interrupts until vblank is enabled */
...
{
	u32 htotal, hbp, hact, vbp, vact, vtotal;
	u32 reg = 0;
	...
	/*
	 * Fire the vertical-threshold IRQ at the start of vertical blanking
	 * (just after the last active line). This is used by the vblank
	 * enable/disable path below.
	 */
	vo->vtth_line = vact;

	/* HSYNC/VSYNC: internal DPI interface timing */
	...
	canaan_vo_write(vo, VO_DISP_DRAW_CTL, reg);

	/*
	 * Note: VO_DISP_IRQ1_CTL holds the vertical-threshold IRQ position and
	 * is programmed by canaan_vo_set_vtth_intr() on vblank enable, so it is
	 * intentionally not written here. The previous code wrote a log2(vtotal)
	 * value into this register from set_timing(), which was immediately
	 * clobbered by the vblank-enable path and is not present in the vendor
	 * bring-up code.
	 */
}
...
{
	if (vo->rstc) {
		reset_control_assert(vo->rstc);
		usleep_range(10, 20);
		reset_control_deassert(vo->rstc);
		usleep_range(10, 20);
	}
	...
	/* White background, shown where no plane is active */
...
{
	/*
	 * Nothing to do: the planes are torn down via their own atomic_disable
	 * hooks and the VO block is fully reprogrammed on the next enable.
	 */
}
...
static const u32 video_plane_formats[] = {
	DRM_FORMAT_NV12,
	DRM_FORMAT_NV21,
	DRM_FORMAT_NV16,
	DRM_FORMAT_NV61,
...
static const u32 osd_plane_formats[] = {
	DRM_FORMAT_XRGB8888, DRM_FORMAT_ARGB4444,
...
static int canaan_vo_bind(struct device *dev, struct device *master, void *data)
{
	...
	vo->rstc = devm_reset_control_get_optional_exclusive(dev, NULL);
	if (IS_ERR(vo->rstc))
		return dev_err_probe(dev, PTR_ERR(vo->rstc),
				     "Failed to get reset control\n");

	atomic_set(&vo->vsync_enabled, 0);
	vo->irq = platform_get_irq(pdev, 0);
	if (vo->irq < 0) {
		dev_err(dev, "Failed to get vsync irq\n");
		return vo->irq;
	}
	...
	for (i = 0; i < CANAAN_PLANE_NUMBER; i++) {
		...
		config->possible_crtcs = BIT(0); /* single CRTC at index 0 */
		vo->canaan_plane[i] = canaan_plane_create(drm_dev, config, vo);
		if (IS_ERR(vo->canaan_plane[i])) {
			dev_err(dev, "Failed to create canaan_plane\n");
	...
	if (IS_ERR(vo->canaan_crtc)) {
		dev_err(dev, "Failed to create canaan_crtc\n");
...
{
	/*
	 * All resources are devm-managed and the DRM objects are torn down by
	 * the DRM core, so there is nothing to undo here.
	 */
}
...
static void canaan_vo_remove(struct platform_device *pdev)
{
...
struct platform_driver canaan_vo_driver = {
	...
	.driver = {
		.name = "canaan-vo",
		.of_match_table = canaan_vo_of_table,
	},
};
